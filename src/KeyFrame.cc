...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB)
    ...
    mbToBeErased(false), mImg(F.imgLeft), mbBad(false), mHalfBaseline(F.mb / 2), mpMap(pMap), mbCurrentPlaceRecognition(false), mNameFile(F.mNameFile), mnMergeCorrectedForKF(0),
...
{
    cout<<"compute keyframe bow!"<<endl;
...
void KeyFrame::ComputeBoW3()
{
    ...
    if(mBow3Vec.empty() || mFeat3Vec.empty())
    {
        ...
        //vocab->load("/media/xiao/data3/learning-slam/ORB_SLAM3_detailed_comments/Vocabulary/voc_binary_tartan_8u_6.yml.gz");
        // cv::Mat randomMat(500, 256, CV_8U);
        // int mathType = mDescriptors.type();//mDescriptors为CV_32F
        // cv::randu(randomMat, cv::Scalar(0), cv::Scalar(255));
        vector<cv::Mat> randomMatDesc = Converter::toDescriptorVector(mDescriptors_bin);
        //vocab->transform(randomMatDesc, mBow3Vec, mFeat3Vec, 4);
        // 将特征点的描述子转换成词袋向量mBowVec以及特征向量mFeatVec
        mpSPvocabulary->transform(randomMatDesc,    //当前的描述子vector
                                   mBow3Vec,         //输出，词袋向量，记录的是单词的id及其对应权重TF-IDF值
                                   mFeat3Vec,        //输出，记录node id及其对应的图像 feature对应的索引
                                   4);              //4表示从叶节点向前数的层数
        // mpSPvocabulary->transform(mDescriptors,    //当前的描述子vector
        //                            mBow3Vec);              //4表示从叶节点向前数的层数
    }    
}
