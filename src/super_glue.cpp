...
bool SuperGlue::build() {
    ...
    std::cout << "Building superGlue engine..." << std::endl;
...
bool SuperGlue::construct_network(TensorRTUniquePtr<nvinfer1::IBuilder> &builder,
    ...
    std::cout<<"superglue construct network!"<<std::endl;
    auto parsed = parser->parseFromFile(superglue_config_.onnx_file.c_str(),
                                        static_cast<int>(gLogger.getReportableSeverity()));
    std::cout<<"superglue construct network success !"<<std::endl;
    if (!parsed) {
        return false;
    }
...
bool SuperGlue::deserialize_engine() {
    std::cout << "deserialize_engine_SuperGlue" << std::endl;
